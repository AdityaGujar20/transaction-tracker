...
document.addEventListener("DOMContentLoaded", function () {
  const analysisCards = document.querySelectorAll(
    ".analysis-card[data-endpoint]"
  );
...
function showStatusMessage(message, type = "") {
  ...
  if (type === "success") {
    setTimeout(() => {
      if (
        statusMessage.classList.contains("show") &&
        statusMessage.classList.contains("success")
      ) {
...
function savePageState() {
  const state = {
    analysisVisible: analysisSection.classList.contains("show"),
  ...
  if (savedState) {
    try {
      ...
      if (Date.now() - state.timestamp < 300000) {
        ...
        if (state.statusMessage && state.statusType) {
          statusMessage.textContent = state.statusMessage;
          statusMessage.className = state.statusType;
          if (state.statusType.includes("show")) {
            statusMessage.classList.add("show");
          }
        }
      }
    } catch (e) {
      console.log("Could not restore page state:", e);
    }
  }
}
